...
namespace CryptographyLibrary
{
	...
	{
		public static void PrintWithSpacesBtwnChars(string text)
		{
		...
		public static string printCipherAlphaWithAlpha(string ciphertext, string partialPlain, string alphabet = "abcdefghiklmnopqrstuvwxyz")
		{
        ...
        public static string ConvertToAlphaPatterns(string ciphertext)
        {
            string alphaPattern = "";
            string alphabet = "abcdefghijklmnopqrstuvwxyz";
            var patterns = new Dictionary<char, char>();
            int i = 0;
            ...
            foreach(var c in ciphertext)
            {
                ...
                {
                    patterns.Add(c, alphabet[i]);
                    ++i;
                }
                alphaPattern += patterns[c];
            }

            return alphaPattern;
        }

        public static string ConvertToNumberPattern(string ciphertext)
        {
            string numberPattern = "";
            var dict = new Dictionary<string, int>();
            int counter = 0;

            foreach(var pair in ciphertext.Split(' '))
            {
                if(!dict.ContainsKey(pair))
                {
                    dict.Add(pair, counter);
                    ++counter;
                }

                numberPattern += dict[pair] + " ";
            }
